...
import org.apache.spark.mllib.clustering.{ArrayBasedKmeans => MLlibKMeans, ArrayBasedKMeansModel => MLlibKMeansModel}
 ...
 */
private[clustering] trait ArrayBasedKMeansParams extends Params with HasMaxIter with HasFeaturesCol
 ...
 */
@Since("1.5.0")
class ArrayBasedKMeansModel private[ml] (
    @Since("1.5.0") override val uid: String,
    private val parentModel: MLlibKMeansModel)
  extends Model[ArrayBasedKMeansModel] with ArrayBasedKMeansParams with MLWritable {

  @Since("1.5.0")
  override def copy(extra: ParamMap): ArrayBasedKMeansModel = {
    val copied = copyValues(new ArrayBasedKMeansModel(uid, parentModel), extra)
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    val predictUDF = udf((vector: Array[Double]) => predict(vector))
  ...
  private[clustering] def predict(features: Array[Double]): Int = parentModel.predict(features)

  @Since("2.0.0")
  def clusterCenters: Array[Array[Double]] = parentModel.clusterCenters
  ...
  def computeCost(dataset: Dataset[_]): Double = {
    ...
    val data: RDD[Array[Double]] = dataset.select(col($(featuresCol))).rdd.map {
      case Row(point: Array[Double]) => point
    }
  ...
  /**
    ...
    */
  @Since("1.6.0")
  override def write: MLWriter = new ArrayBasedKMeansModel.KMeansModelWriter(this)
...
@Since("1.6.0")
object ArrayBasedKMeansModel extends MLReadable[KMeansModel] {
  ...
  private case class Data(clusterIdx: Int, clusterCenter: Array[Double])
  ...
  /**
    ...
    */
  private case class OldData(clusterCenters: Array[Array[Double]])

  /** [[MLWriter]] instance for [[KMeansModel]] */
  private[KMeansModel] class KMeansModelWriter(instance: ArrayBasedKMeansModel) extends MLWriter {
  ...
  private class KMeansModelReader extends MLReader[KMeansModel] {
    ...
    override def load(path: String): ArrayBasedKMeansModel = {
      ...
      val model = new ArrayBasedKMeansModel(metadata.uid, new MLlibKMeansModel(clusterCenters))
 ...
 */
@Since("1.5.0")
class ArrayBasedKMeans @Since("1.5.0") (
                               @Since("1.5.0") override val uid: String)
  extends Estimator[ArrayBasedKMeansModel] with KMeansParams with DefaultParamsWritable {
  ...
  @Since("2.0.0")
  override def fit(dataset: Dataset[_]): ArrayBasedKMeansModel = {
    transformSchema(dataset.schema, logging = true)

    val handlePersistence = dataset.rdd.getStorageLevel == StorageLevel.NONE
    val instances: RDD[Array[Double]] = dataset.select(col($(featuresCol))).rdd.map {
      case Row(point: Array[Double]) => point
    }
    ...
    val model = copyValues(new ArrayBasedKMeansModel(uid, parentModel).setParent(this))
...
@Since("1.6.0")
object ArrayBasedKMeans extends DefaultParamsReadable[ArrayBasedKMeans] {

  @Since("1.6.0")
  override def load(path: String): ArrayBasedKMeans = super.load(path)
}
