...
import org.apache.spark.ml.clustering.{ArrayBasedKMeans => NewKMeans}
 ...
 */
@Since("0.8.0")
class ArrayBasedKmeans private (
     private var k: Int,
  ...
  private var initialModel: Option[ArrayBasedKMeansModel] = None
  ...
  /**
    ...
    */
  @Since("1.4.0")
  def setInitialModel(model: ArrayBasedKMeansModel): this.type = {
  ...
  /**
    ...
    */
  @Since("0.8.0")
  def run(data: RDD[Array[Double]]): ArrayBasedKMeansModel = {
    run(data, None)
  }

  private[spark] def run(
      data: RDD[Array[Double]],
      instr: Option[Instrumentation[NewKMeans]]): ArrayBasedKMeansModel = {
    ...
    norms.persist(StorageLevel.OFF_HEAP)
    val zippedData = data.zip(norms).map { case (v, norm) =>
      new ArrayWithNorm(v, norm)
    }
  ...
  /**
    ...
    */
  private def runAlgorithm(data: RDD[ArrayWithNorm],
      instr: Option[Instrumentation[NewKMeans]]): ArrayBasedKMeansModel = {
    ...
    val centers = initialModel match {
      case Some(kMeansCenters) =>
        kMeansCenters.clusterCenters.map(new ArrayWithNorm(_))
      case None =>
    ...
    while (iteration < maxIterations && !converged) {
      ...
      val newCenters = data.mapPartitions { points =>
        ...
        val sums = Array.fill(thisCenters.length)(new Array[Double](dims))
        val counts = Array.fill(thisCenters.length)(0L)

        points.foreach { point =>
          val (bestCenter, cost) = ArrayBasedKmeans.findClosest(thisCenters, point)
          costAccum.add(cost)
      ...
      newCenters.foreach { case (j, newCenter) =>
        if (converged &&
          ArrayBasedKmeans.fastSquaredDistance(newCenter, centers(j)) > epsilon * epsilon) {
          converged = false
        }
        centers(j) = newCenter
      }
    ...
    new ArrayBasedKMeansModel(centers.map(_.vector))
  }

  /**
    * Initialize a set of cluster centers at random.
    */
  private def initRandom(data: RDD[ArrayWithNorm]): Array[ArrayWithNorm] = {
    ...
    data.takeSample(false, k, new XORShiftRandom(this.seed).nextInt())
      .map(_.vector).distinct.map(new ArrayWithNorm(_))
  }
  ...
  /**
    ...
    */
  private[clustering] def initKMeansParallel(data: RDD[ArrayWithNorm]): Array[ArrayWithNorm] = {
    ...
    val centers = ArrayBuffer[ArrayWithNorm]()
    ...
    while (step < initializationSteps) {
      ...
      costs = data.zip(preCosts).map { case (point, cost) =>
        math.min(ArrayBasedKmeans.pointCost(bcNewCenters.value, point), cost)
    ...
    val distinctCenters = centers.map(_.vector).distinct.map(new ArrayWithNorm(_))
    ...
    if (distinctCenters.size <= k) {
      ...
      val countMap = data.map(ArrayBasedKmeans.findClosest(bcCenters.value, _)._1).countByValue()
...
object ArrayBasedKmeans {
  ...
  private[mllib] def findClosest(
      centers: TraversableOnce[ArrayWithNorm],
      point: ArrayWithNorm): (Int, Double) = {
 ...
 */
private[clustering]
class ArrayWithNorm(val vector: Array[Double], val norm: Double) extends Serializable {

  def this(vector: Array[Double]) = this(vector, Vectors.norm(vector, 2.0))

//  /** Converts the vector to a dense vector. */
//  def toDense: VectorWithNorm = new VectorWithNorm(Vectors.dense(vector), norm)
}
