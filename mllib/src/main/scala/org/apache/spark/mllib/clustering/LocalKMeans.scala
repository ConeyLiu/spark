...
private[mllib] object LocalKMeans extends Logging {
  ...
  def kMeansPlusPlus(
      seed: Int,
      points: Array[ArrayWithNorm],
      weights: Array[Double],
      k: Int,
      maxIterations: Int): Array[ArrayWithNorm] = {
    val rand = new Random(seed)
    val dimensions = points(0).vector.size
    val centers = new Array[ArrayWithNorm](k)

    // Initialize centers by sampling using the k-means++ procedure.
    centers(0) = pickWeighted(rand, points, weights)
    val costArray = points.map(ArrayBasedKmeans.fastSquaredDistance(_, centers(0)))
    ...
    for (i <- 1 until k) {
      ...
      for (p <- points.indices) {
        costArray(p) =
          math.min(ArrayBasedKmeans.fastSquaredDistance(points(p), centers(i)), costArray(p))
      }
    ...
    while (moved && iteration < maxIterations) {
      moved = false
      val counts = Array.fill(k)(0.0)
      val sums = Array.fill(k)(new Array[Double](dimensions))
      var i = 0
      while (i < points.length) {
        val p = points(i)
        val index = ArrayBasedKmeans.findClosest(centers, p)._1
        axpy(weights(i), p.vector, sums(index))
        counts(index) += weights(i)
        if (index != oldClosest(i)) {
          moved = true
      ...
      while (j < k) {
        if (counts(j) == 0.0) {
          ...
          scal(1.0 / counts(j), sums(j))
          centers(j) = new ArrayWithNorm(sums(j))
        }
        j += 1
      }
      iteration += 1
